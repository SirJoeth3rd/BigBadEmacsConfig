...
(defun whitebox-connect nil
  ...
  (unless (whitebox-status)
    (ignore-errors (make-network-process :name "whitebox" :buffer "*whitebox*" :family 'ipv4
                                         :host whitebox-host
     ; HACK(Phillip): I'm not experienced with Lisp or Emacs so we are completely overriding
     ; the whitebox port customization and we're not error-checking failure
     ; NOTE: how do we *not* override whitebox-port if the folder doesn't exist?
     ;                                              if the file doesn't exist?
     ;                                              if the file contains a non-integer?
     ;                                              if the integer is not in 1..65535?
     ; :service (eval (car (read-from-string (with-temp-buffer (insert-file-contents (concat (getenv "HOME") "/.whitebox/port")) (buffer-string)))))
     :service whitebox-port
     ))
